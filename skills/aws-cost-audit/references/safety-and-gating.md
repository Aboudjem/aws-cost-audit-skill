...
- **Deregister-AMI-before-snapshot-delete.** An AMI and its backing snapshots are linked; deregister
  the AMI before deleting the snapshots so the operation is ordered and inspectable. (The end state —
  no AMI, no snapshot — is still irreversible; this only makes the path deliberate.)
